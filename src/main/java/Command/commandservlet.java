...
public class commandservlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out4 = response.getWriter()) {
            String us=request.getParameter("nuevous");
            String pass=request.getParameter("nuevopass");
            String email=request.getParameter("nuevoemail");
            Registrarimpl opRegistro = new Registrarimpl(us, pass, email);

            Invoker ivk = new Invoker();
            ivk.recibirOperacion(opRegistro);
            String resp=ivk.realizarOperaciones();
            if(resp.equalsIgnoreCase("agregado")){
